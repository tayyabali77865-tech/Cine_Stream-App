const axios = require('axios');
const crypto = require('crypto');
const cheerio = require('cheerio');

const secret = 'net###@@sss';
const domains = [
  'speed.watch22.shop',
  'play.watch22.shop',
  'play.watch21.shop',
  'test.watch22.shop',
  'playnew.watch21.shop',
  'speed.watch21.shop'
];

async function test() {
  const id = '103375';
  const se = '10';
  const ep = '9';
  const dp = 'OURXd1pWZ3ZKNTJxSmxUc3NpRTZvd0RObHpIYWlEakZPbVNSQmExSFlNQnd6QmxTcTl4MEorWURxaDhMVkN4ZQ==';
  const title = 'Naruto: Shippuden [Bengali]';
  const na = Buffer.from(title).toString('base64');
  
  for (const domain of domains) {
    const watchboxBaseUrl = `https://${domain}/play/watchbox.php?id=${id}&se=${se}&ep=${ep}&dp=${dp}&na=${encodeURIComponent(na)}&exten=1`;
    const referer = 'https://netmirror.global/';
    
    try {
      console.log(`📡 Trying domain: ${domain}...`);
      // 1. Sync Time
      const dummyRes = await axios.get(`${watchboxBaseUrl}&ts=0&sig=0`, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
          'Referer': referer
        }
      });
      
      const timeMatch = dummyRes.data.match(/Time not Found\.<br><br>(\d+)/);
      if (!timeMatch) {
        console.log(`No time on ${domain}`);
        continue;
      }
      
      const serverTime = timeMatch[1];
      const signature = crypto.createHmac('sha256', secret).update(String(serverTime)).digest('hex');
      const authUrl = `${watchboxBaseUrl}&ts=${serverTime}&sig=${signature}`;
      
      const finalRes = await axios.get(authUrl, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
          'Referer': referer
        }
      });
      
      const html = finalRes.data;
      if (html.includes('Server Buzy') || html.includes('Not Found')) {
        console.log(`❌ Domain ${domain} returned: Busy or Not Found`);
      } else {
        console.log(`✅ SUCCESS on ${domain}!`);
        // Find R2.dev or bcdnxw link
        const bcdnMatch = html.match(/(https:\/\/[a-zA-Z0-9.-]+\.hakunaymatata\.com\/[^\'\"]+\.mp4\?[^\'\"]+)/);
        const r2Match = html.match(/(https:\/\/[a-zA-Z0-9.-]+\.r2\.dev\/[^\'\"]+\?[^\'\"]+)/);
        console.log('Resolved URL:', bcdnMatch ? bcdnMatch[1] : (r2Match ? r2Match[1] : 'Not Found in HTML'));
        break;
      }
    } catch (e) {
      console.log(`Failed for ${domain}:`, e.message);
    }
  }
}

test();
